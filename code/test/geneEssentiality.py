...
data/testResults/:
  ...
  * gene-essential.csv          the detailed per-gene essentiality matrix (all
                                model genes as rows, cell lines as columns,
                                yes/no), diff-friendly for comparing runs.
...
def main() -> int:
    ...
    # Detailed per-gene essentiality matrix (the diffable artifact).
    MATRIX_CSV.write_text(
        essentiality_matrix_csv(egenes["gene_ids"], symbol_of, tissues, essential_ids_by_tissue),
        encoding="utf-8",
    )

    # Summary statistics against the Hart 2015 data. The comparison is done in
    # Ensembl space (see issue #970): Hart's gene symbols are matched to model
    # genes through their symbols and aliases, so essential gene ids are passed
    # through directly rather than collapsed to a single symbol.
